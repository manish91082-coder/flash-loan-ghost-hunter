...
class ExecutionIntentBuilder:
    ...
    def build_calldata(self, intent_dict):
        ...
        intent_tuple = (
            signed["executionId"], signed["providerType"],
            self.w3.to_checksum_address(signed["providerAddress"]),
            self.w3.to_checksum_address(signed["tokenBorrow"]), signed["amountBorrow"],
            signed["swap1Type"], self.w3.to_checksum_address(signed["routerA"]),
            signed["pathA"], signed["minAmountOut1"], signed["swap2Type"],
            self.w3.to_checksum_address(signed["routerB"]), signed["pathB"],
            signed["minAmountOutFinal"], signed["minimumOnChainSurplus"],
            signed["maximumGasLimit"], signed["deadline"], signed["signature"]
        )
...
class PathEncoder:
    ...
    def build_v3_path_triangular(token_a, fee1, token_b, fee2, token_c, fee3, token_end):
        return (
            Web3.to_bytes(hexstr=token_a) + int(fee1).to_bytes(3, 'big') + Web3.to_bytes(hexstr=token_b) +
            int(fee2).to_bytes(3, 'big') + Web3.to_bytes(hexstr=token_c) +
            int(fee3).to_bytes(3, 'big') + Web3.to_bytes(hexstr=token_end)
        )
