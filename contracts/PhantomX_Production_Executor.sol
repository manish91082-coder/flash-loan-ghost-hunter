...
contract PhantomX_Production_Executor {
    ...
    bytes32 internal activeIntentHash;
    uint256 internal activeInitialBalance;
    bool internal activeCallbackConsumed;
    address internal trustedUniswapV3Factory;

    struct ExecutionIntent {
        bytes32 executionId; FlashProviderType providerType; address providerAddress; address tokenBorrow; uint256 amountBorrow;
        SwapType swap1Type; address routerA; bytes pathA; uint256 minAmountOut1; SwapType swap2Type; address routerB; bytes pathB;
        uint256 minAmountOutFinal; uint256 minimumOnChainSurplus; uint256 maximumGasLimit; uint256 deadline; bytes signature;
    }
    ...
    function setUniswapV3Factory(address f) external onlyOwner{ require(f!=address(0),"Zero address"); trustedUniswapV3Factory=f; }
    ...
    function _verifyIntentSignature(ExecutionIntent memory intent) internal view returns(bool){ bytes32 digest=_intentDigest(intent); require(intent.signature.length==65,"Invalid sig length"); bytes32 r;bytes32 s;uint8 v;bytes memory sig=intent.signature; assembly{r:=mload(add(sig,32)) s:=mload(add(sig,64)) v:=byte(0,mload(add(sig,96)))} require(v==27||v==28,"Invalid sig v"); require(uint256(s)<=0x7fffffffffffffffffffffffffffffff5d576e7357a4501ddfe92f46681b20a0,"Invalid sig s"); return ecrecover(digest,v,r,s)==owner; }

    function executeOpportunity(ExecutionIntent memory intent) external notPaused nonReentrant {
        uint256 gasStart=gasleft(); require(_verifyIntentSignature(intent),"Invalid Intent Signature"); require(block.timestamp<=intent.deadline,"Expired"); require(!usedIntents[intent.executionId],"Intent replayed"); require(intent.amountBorrow>0,"Zero borrow amount"); require(intent.maximumGasLimit>0,"Zero gas limit"); require(intent.minAmountOut1>0&&intent.minAmountOutFinal>0,"Zero minimum output"); require(intent.providerAddress!=address(0)&&intent.tokenBorrow!=address(0)&&intent.routerA!=address(0)&&intent.routerB!=address(0),"Zero execution address"); require(uint8(intent.providerType)<=uint8(FlashProviderType.BALANCER),"Invalid provider type"); require(uint8(intent.swap1Type)<=uint8(SwapType.V3)&&uint8(intent.swap2Type)<=uint8(SwapType.V3),"Invalid swap type");
        usedIntents[intent.executionId]=true; bytes memory params=abi.encode(intent); activeExecutionId=intent.executionId; activeIntentHash=_intentStructHash(intent); activeInitialBalance=IERC20(intent.tokenBorrow).balanceOf(address(this)); activeCallbackConsumed=false;
        ...
        require(activeCallbackConsumed,"Flash callback missing"); activeExecutionId=bytes32(0); activeIntentHash=bytes32(0); activeInitialBalance=0; activeCallbackConsumed=false; require(gasStart-gasleft()<=intent.maximumGasLimit,"Gas limit exceeded");
    }
    ...
    function uniswapV3FlashCallback(uint256 fee0,uint256 fee1,bytes calldata data) external { require(!activeCallbackConsumed,"Callback already consumed"); activeCallbackConsumed=true; require(activeExecutionId!=bytes32(0),"No active execution"); ExecutionIntent memory intent=abi.decode(data,(ExecutionIntent)); require(intent.providerType==FlashProviderType.UNISWAP_V3,"Wrong provider type"); require(msg.sender==intent.providerAddress&&isUniswapV3Pool[msg.sender],"Untrusted UniV3 provider"); require(intent.executionId==activeExecutionId,"Active ID mismatch"); require(_intentStructHash(intent)==activeIntentHash,"Active intent mismatch"); address token0=IUniswapV3Pool(msg.sender).token0(); address token1=IUniswapV3Pool(msg.sender).token1(); require(trustedUniswapV3Factory!=address(0),"UniV3 factory not configured"); (bool factoryOk,bytes memory factoryData)=msg.sender.staticcall(abi.encodeWithSignature("factory()")); require(factoryOk&&factoryData.length>=32,"UniV3 pool factory missing"); address observedFactory=abi.decode(factoryData,(address)); require(observedFactory==trustedUniswapV3Factory,"Untrusted UniV3 factory"); (bool feeOk,bytes memory feeData)=msg.sender.staticcall(abi.encodeWithSignature("fee()")); require(feeOk&&feeData.length>=32,"UniV3 pool fee missing"); uint24 poolFee=abi.decode(feeData,(uint24)); (bool poolOk,bytes memory poolData)=trustedUniswapV3Factory.staticcall(abi.encodeWithSignature("getPool(address,address,uint24)",token0,token1,poolFee)); require(poolOk&&poolData.length>=32&&abi.decode(poolData,(address))==msg.sender,"Non-canonical UniV3 pool"); require(intent.tokenBorrow==token0||intent.tokenBorrow==token1,"Asset mismatch"); if(intent.tokenBorrow==token0)require(fee1==0,"Unexpected token1 flash fee"); else require(fee0==0,"Unexpected token0 flash fee"); uint256 fee=intent.tokenBorrow==token0?fee0:fee1; uint256 balanceBefore=IERC20(intent.tokenBorrow).balanceOf(address(this)); require(balanceBefore>=activeInitialBalance+intent.amountBorrow,"Flash amount not received"); uint256 totalRepayment=intent.amountBorrow+fee; uint256 finalAmount=_executeRoute(intent); require(finalAmount>=totalRepayment+intent.minimumOnChainSurplus,"MinProfit failed"); require(IERC20(intent.tokenBorrow).balanceOf(address(this))>=activeInitialBalance+totalRepayment+intent.minimumOnChainSurplus,"Insufficient balance/profit"); IERC20(intent.tokenBorrow).safeTransfer(msg.sender,totalRepayment); }
