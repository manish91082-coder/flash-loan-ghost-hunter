...
contract PhantomX_Production_Executor {
    ...
    function _intentStructHash(ExecutionIntent memory intent) internal pure returns(bytes32){
        ...
        words[0]=EXECUTION_INTENT_TYPEHASH;
    ...
    function executeOpportunity(ExecutionIntent memory intent) external notPaused nonReentrant {
        ...
        if(intent.providerType==FlashProviderType.AAVE){
            ...
            IPool(intent.providerAddress).flashLoanSimple(address(this),intent.tokenBorrow,intent.amountBorrow,params,0);
            ...
            IUniswapV3Pool(intent.providerAddress).flash(address(this),amount0,amount1,params);
            ...
            address[] memory tokens=new address[](1);tokens[0]=intent.tokenBorrow;
            uint256[] memory amounts=new uint256[](1);amounts[0]=intent.amountBorrow;
            IBalancerVault(intent.providerAddress).flashLoan(address(this),tokens,amounts,params);
        } else revert("Unknown Provider");
        activeExecutionId=bytes32(0);
        activeIntentHash=bytes32(0);
        activeInitialBalance=0;
        require(gasStart-gasleft()<=intent.maximumGasLimit,"Gas limit exceeded");
    }

    function _validateV2Path(bytes memory path,address borrowToken,bool finalLeg) internal pure returns(address[] memory p){
        p=abi.decode(path,(address[]));
        require(p.length>=2,"V2 path too short");
        require(p[0]!=address(0),"V2 path starts zero");
        if(!finalLeg)require(p[0]==borrowToken,"V2 leg1 input mismatch");
        if(finalLeg)require(p[p.length-1]==borrowToken,"V2 final token mismatch");
    }
    function _validateV3Path(bytes memory path,address borrowToken,bool finalLeg) internal pure {
        require(path.length>=43&&(path.length-20)%23==0,"V3 path malformed");
        address first;address last;
        assembly{first:=shr(96,mload(add(path,32))) last:=shr(96,mload(add(path,add(32,sub(mload(path),20)))))}
        if(!finalLeg)require(first==borrowToken,"V3 leg1 input mismatch");
        if(finalLeg)require(last==borrowToken,"V3 final token mismatch");
    }
    function _firstToken(SwapType t,bytes memory path) internal pure returns(address){
        if(t==SwapType.V2){address[] memory p=abi.decode(path,(address[]));require(p.length>=2,"V2 path too short");return p[0];}
        if(t==SwapType.V3){require(path.length>=43&&(path.length-20)%23==0,"V3 path malformed");bytes32 w;assembly{w:=mload(add(path,32))}return address(bytes20(w));}
        revert("Invalid SwapType");
    }
    function _lastToken(SwapType t,bytes memory path) internal pure returns(address){
        if(t==SwapType.V2){address[] memory p=abi.decode(path,(address[]));require(p.length>=2,"V2 path too short");return p[p.length-1];}
        if(t==SwapType.V3){require(path.length>=43&&(path.length-20)%23==0,"V3 path malformed");bytes32 w;uint256 n;assembly{n:=mload(path) w:=mload(add(path,add(32,sub(n,20))))}return address(bytes20(w));}
        revert("Invalid SwapType");
    }
    function _validateAllowedTokens(SwapType t,bytes memory path) internal view {
        if(t==SwapType.V2){address[] memory p=abi.decode(path,(address[]));require(p.length>=2,"V2 path too short");for(uint256 i=0;i<p.length;i++){require(allowedTokens[p[i]],"Unauthorized route token");}}
        else if(t==SwapType.V3){require(path.length>=43&&(path.length-20)%23==0,"V3 path malformed");uint256 n=path.length;for(uint256 offset=0;offset<=n-20;offset+=23){address token;assembly{token:=shr(96,mload(add(add(path,32),offset)))}require(allowedTokens[token],"Unauthorized route token");}}
        else revert("Invalid SwapType");
    }
    function _executeRoute(ExecutionIntent memory intent) internal returns(uint256 finalAmount){
        ...
        _validateAllowedTokens(intent.swap1Type,intent.pathA);
        _validateAllowedTokens(intent.swap2Type,intent.pathB);
        ...
        uint256 out1=_swap(intent.swap1Type,intent.routerA,intent.pathA,intent.amountBorrow,intent.minAmountOut1,intent.deadline);
        finalAmount=_swap(intent.swap2Type,intent.routerB,intent.pathB,out1,intent.minAmountOutFinal,intent.deadline);
        return finalAmount;
    }
    function _swap(SwapType t,address router,bytes memory path,uint256 amountIn,uint256 minAmountOut,uint256 deadline) internal returns(uint256){
        address tokenIn=_firstToken(t,path);
        IERC20(tokenIn).safeApprove(router,amountIn);
        uint256 amountOut;
        if(t==SwapType.V2){
            address[] memory p=abi.decode(path,(address[]));
            uint[] memory amounts=IUniswapV2Router(router).swapExactTokensForTokens(amountIn,minAmountOut,p,address(this),deadline);
            amountOut=amounts[amounts.length-1];
        } else {
            ISwapRouter.ExactInputParams memory params=ISwapRouter.ExactInputParams({path:path,recipient:address(this),deadline:deadline,amountIn:amountIn,amountOutMinimum:minAmountOut});
    ...
    function executeOperation(address asset,uint256 amount,uint256 premium,address initiator,bytes calldata params) external returns(bool){
        require(msg.sender==_activeAaveProvider(params),"Untrusted callback");
        ...
        require(intent.providerType==FlashProviderType.AAVE,"Wrong provider type");
        require(msg.sender==intent.providerAddress&&isAavePool[msg.sender],"Untrusted Aave provider");
        require(intent.executionId==activeExecutionId,"Active ID mismatch");
        require(_intentStructHash(intent)==activeIntentHash,"Active intent mismatch");
        require(asset==intent.tokenBorrow&&amount==intent.amountBorrow,"Asset/amount mismatch");
        uint256 balanceBefore=IERC20(asset).balanceOf(address(this));
        require(balanceBefore>=activeInitialBalance+intent.amountBorrow,"Flash amount not received");
        ...
        require(IERC20(asset).balanceOf(address(this))>=activeInitialBalance+totalRepayment+intent.minimumOnChainSurplus,"Insufficient balance/profit");
    ...
    function receiveFlashLoan(address[] memory tokens,uint256[] memory amounts,uint256[] memory feeAmounts,bytes memory userData) external {
        ...
        require(intent.providerType==FlashProviderType.BALANCER,"Wrong provider type");
        require(msg.sender==intent.providerAddress&&isBalancerVault[msg.sender],"Untrusted Balancer provider");
        require(intent.executionId==activeExecutionId,"Active ID mismatch");
        require(_intentStructHash(intent)==activeIntentHash,"Active intent mismatch");
        require(tokens.length==1&&amounts.length==1&&feeAmounts.length==1,"Flash loan shape mismatch");
        require(tokens[0]==intent.tokenBorrow&&amounts[0]==intent.amountBorrow,"Asset/amount mismatch");
        uint256 balanceBefore=IERC20(tokens[0]).balanceOf(address(this));
        require(balanceBefore>=activeInitialBalance+intent.amountBorrow,"Flash amount not received");
        uint256 finalAmount=_executeRoute(intent);
        uint256 totalRepayment=amounts[0]+feeAmounts[0];
        require(finalAmount>=totalRepayment+intent.minimumOnChainSurplus,"MinProfit failed");
        require(IERC20(tokens[0]).balanceOf(address(this))>=activeInitialBalance+totalRepayment+intent.minimumOnChainSurplus,"Insufficient balance/profit");
    ...
    function uniswapV3FlashCallback(uint256 fee0,uint256 fee1,bytes calldata data) external {
        ...
        require(intent.providerType==FlashProviderType.UNISWAP_V3,"Wrong provider type");
        require(msg.sender==intent.providerAddress&&isUniswapV3Pool[msg.sender],"Untrusted UniV3 provider");
        require(intent.executionId==activeExecutionId,"Active ID mismatch");
        require(_intentStructHash(intent)==activeIntentHash,"Active intent mismatch");
        address token0=IUniswapV3Pool(msg.sender).token0();
        address token1=IUniswapV3Pool(msg.sender).token1();
        require(intent.tokenBorrow==token0||intent.tokenBorrow==token1,"Asset mismatch");
        if(intent.tokenBorrow==token0) require(fee1==0,"Unexpected token1 flash fee");
        else require(fee0==0,"Unexpected token0 flash fee");
        ...
        uint256 finalAmount=_executeRoute(intent);
        require(finalAmount>=totalRepayment+intent.minimumOnChainSurplus,"MinProfit failed");
        require(IERC20(intent.tokenBorrow).balanceOf(address(this))>=activeInitialBalance+totalRepayment+intent.minimumOnChainSurplus,"Insufficient balance/profit");
