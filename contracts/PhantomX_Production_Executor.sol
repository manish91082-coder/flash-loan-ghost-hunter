...
contract PhantomX_Production_Executor {
    ...
    function _verifyIntentSignature(ExecutionIntent memory intent) internal view returns (bool) {
        bytes memory encoded1 = abi.encode(
            EXECUTION_INTENT_TYPEHASH, intent.executionId, intent.providerType, intent.providerAddress,
            intent.tokenBorrow, intent.amountBorrow, intent.swap1Type, intent.routerA
        );
        bytes memory encoded2 = abi.encode(
            keccak256(intent.pathA), intent.minAmountOut1, intent.swap2Type, intent.routerB,
            keccak256(intent.pathB), intent.minAmountOutFinal, intent.minimumOnChainSurplus, intent.maximumGasLimit
        );
        bytes memory encoded3 = abi.encode(intent.deadline);
    ...
    function _validateV2Path(bytes memory path, address borrowToken, bool finalLeg) internal pure returns (address[] memory decodedPath) {
        ...
        require(decodedPath.length >= 2, "V2 path too short");
        require(decodedPath[0] != address(0), "V2 path starts zero");
        if (!finalLeg) require(decodedPath[0] == borrowToken, "V2 leg1 input mismatch");
        if (finalLeg) require(decodedPath[decodedPath.length - 1] == borrowToken, "V2 final token mismatch");
    }
    ...
    function _validateV3Path(bytes memory path, address borrowToken, bool finalLeg) internal pure {
        ...
        assembly {
            ...
            last := shr(96, mload(add(path, add(32, sub(mload(path), 20)))))
        }
    ...
    function _executeRoute(ExecutionIntent memory intent) internal returns (uint256 finalAmount) {
        ...
        if (intent.swap1Type == SwapType.V2) {
            _validateV2Path(intent.pathA, intent.tokenBorrow, false);
        } else {
            _validateV3Path(intent.pathA, intent.tokenBorrow, false);
        }
        if (intent.swap2Type == SwapType.V2) {
            _validateV2Path(intent.pathB, intent.tokenBorrow, true);
        } else {
            _validateV3Path(intent.pathB, intent.tokenBorrow, true);
        }
    ...
    function _swap(SwapType swapType, address router, bytes memory path, uint256 amountIn, uint256 minAmountOut, uint256 deadline) internal returns (uint256) {
        ...
        if (swapType == SwapType.V2) {
            ...
            bytes32 firstWord;
            assembly { firstWord := mload(add(path, 32)) }
        ...
        if (swapType == SwapType.V2) {
            address[] memory decodedPath = abi.decode(path, (address[]));
            uint[] memory amounts = IUniswapV2Router(router).swapExactTokensForTokens(amountIn, minAmountOut, decodedPath, address(this), deadline);
            return amounts[amounts.length - 1];
        }
        ISwapRouter.ExactInputParams memory params = ISwapRouter.ExactInputParams({
            path: path, recipient: address(this), deadline: deadline, amountIn: amountIn, amountOutMinimum: minAmountOut
        });
    ...
    function _activeAaveProvider(bytes calldata params) internal view returns (address) {
        ExecutionIntent memory intent = abi.decode(params, (ExecutionIntent));
        return intent.providerAddress;
    }
