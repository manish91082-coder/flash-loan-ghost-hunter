...
# Polygon PoS official deployment addresses, verified from protocol documentation.
...
# Common selectors, generated from canonical Solidity signatures.
TOKEN0 = "0x0dfe1681"
TOKEN1 = "0xd21220a7"
DECIMALS = "0x313ce567"
SYMBOL = "0x95d89b41"
...
# IQuoterV2.quoteExactInputSingle((address,address,uint256,uint24,uint160))
...
def _decode_symbol(result: str) -> str:
    ...
    # Some old ERC20s return bytes32 rather than string.
...
def discover_token_meta(
    *, snapshot: BlockSnapshot, token_address: str,
...
def discover_v2_pair_tokens(
    *, snapshot: BlockSnapshot, pool: str,
    rpc_call_at_block: Callable[[str, str, int], str],
) -> tuple[TokenMeta, TokenMeta]:
...
def discover_v3_pool_meta(
    *, snapshot: BlockSnapshot, pool: str,
    rpc_call_at_block: Callable[[str, str, int], str],
) -> tuple[TokenMeta, TokenMeta, int]:
    token0 = _decode_address(rpc_call_at_block(pool, TOKEN0, snapshot.block_number))
    token1 = _decode_address(rpc_call_at_block(pool, TOKEN1, snapshot.block_number))
...
def _find_stable_and_asset(tokens: tuple[TokenMeta, TokenMeta], stable_symbols: set[str]) -> tuple[TokenMeta, TokenMeta]:
    ...
    raise EconomicTruthError(
        f"No configured stablecoin token in pool; symbols={[t.symbol for t in tokens]}"
    )


def quote_v2_single(
    *, snapshot: BlockSnapshot, router: str, token_in: TokenMeta, token_out: TokenMeta,
    amount_in_raw: int, rpc_call_at_block: Callable[[str, str, int], str],
    gas_units: int,
) -> ExactQuote:
    """Quote one V2 router leg with getAmountsOut at the pinned block."""
    ...
    # dynamic uint[] ABI result: offset(32), length(32), then amounts.
...
def quote_v3_single(
    *, snapshot: BlockSnapshot, quoter: str, token_in: TokenMeta, token_out: TokenMeta,
    amount_in_raw: int, fee: int, rpc_call_at_block: Callable[[str, str, int], str],
) -> ExactQuote:
    """Quote one Uniswap V3 leg with QuoterV2 at the pinned block."""
    # ABI head for tuple(address,address,uint256,uint24,uint160) = 5 words.
    encoded_tuple = (
    ...
    calldata = QUOTE_EXACT_INPUT_SINGLE + encoded_tuple
...
def quote_v2_roundtrip(
    *, snapshot: BlockSnapshot, pool: str, loan_usd: Decimal,
    rpc_call_at_block: Callable[[str, str, int], str],
    ...
    leg1 = quote_v2_single(
        snapshot=snapshot, router=QUICKSWAP_V2_ROUTER, token_in=stable, token_out=asset,
        amount_in_raw=amount_in_raw, rpc_call_at_block=rpc_call_at_block, gas_units=gas_units_leg1,
    )
    leg2 = quote_v2_single(
        snapshot=snapshot, router=QUICKSWAP_V2_ROUTER, token_in=asset, token_out=stable,
        amount_in_raw=leg1.amount_out_raw, rpc_call_at_block=rpc_call_at_block, gas_units=gas_units_leg2,
    )
    return leg1, leg2


def quote_v3_roundtrip(
    *, snapshot: BlockSnapshot, pool: str, loan_usd: Decimal,
    rpc_call_at_block: Callable[[str, str, int], str],
    stable_symbols: set[str] | None = None,
) -> tuple[ExactQuote, ExactQuote]:
    ...
    amount_in_raw = int((loan_usd * (D(10) ** stable.decimals)).to_integral_exact())
    leg1 = quote_v3_single(
        snapshot=snapshot, quoter=UNISWAP_V3_QUOTER_V2, token_in=stable, token_out=asset,
        amount_in_raw=amount_in_raw, fee=fee, rpc_call_at_block=rpc_call_at_block,
    )
    leg2 = quote_v3_single(
        snapshot=snapshot, quoter=UNISWAP_V3_QUOTER_V2, token_in=asset, token_out=stable,
        amount_in_raw=leg1.amount_out_raw, fee=fee, rpc_call_at_block=rpc_call_at_block,
    )
    return leg1, leg2
