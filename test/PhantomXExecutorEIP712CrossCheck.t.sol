...
contract PhantomXExecutorEIP712CrossCheckTest {
    VmEIP712 internal constant vm = VmEIP712(address(uint160(uint256(keccak256("hevm cheat code")))));

    uint256 internal constant PRIVATE_KEY = 0x0123456789012345678901234567890123456789012345678901234567890123;
    address internal ownerAddress;
    PhantomXExecutorEIP712Harness internal executor;
    ...
    function _intent() internal view returns (PhantomX_Production_Executor.ExecutionIntent memory intent) {
        ...
        intent.pathA = abi.encode(new address[](2));
        intent.minAmountOut1 = 120000000;
        intent.swap2Type = PhantomX_Production_Executor.SwapType.V3;
    ...
    function test_solidity_eip712_recovery_matches_owner() public {
        executor = _deploy();
        ...
        require(executor.verify(intent), "valid signature rejected");
    }

    function test_solidity_eip712_rejects_mutated_intent() public {
        executor = _deploy();
        PhantomX_Production_Executor.ExecutionIntent memory intent = _intent();
        bytes32 digest = _digest(intent);
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(PRIVATE_KEY, digest);
